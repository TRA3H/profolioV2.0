...
const GPTCyrus = () => {
  ...
  const [messages, setMessages] = useState<{ type: 'user' | 'response', content: string }[]>([
    { type: 'response', content: `Hello, I'm Cyrus. How can I assist you today?` },
  ]);
  ...
  return (
    <div className=" min-h-screen flex flex-col items-center justify-center text-white">
      <div className="text-4xl text-white font-bold leading-tight text-center ">
          <h1 className="mb-10">
            Say Hello to <br /><span className="text-yellow-300">CyrusGPT</span>.
          </h1>
      </div>
      <div className="rounded-lg px-8 py-6 shadow-md w-full md:w-1/2">
        <h1 className="text-2xl font-bold mb-2">Chat with CyrusGPT</h1>
        <p className="text-gray-400 mb-4">Powered by OpenAI&apos;s GPT-3.5 Turbo.</p>
            ...
            {messages.map((message, index) => (
              <div key={`msg-${index}`} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-1/2 p-2 rounded-lg break-words ${message.type === 'user' ? 'bg-blue-600' : 'bg-gray-600'}`}>
                  <span className="font-semibold">{message.type === 'user' ? 'You:' : 'CyrusGPT:'}</span> {message.content}
                </div>
              </div>
            ))}
            {loading && (
        ...
        <form onSubmit={handleSubmit} className="flex items-center">
          <div className="relative flex-grow">
            <input
              type="text"
              placeholder="Ask me anything about Cyrus..."
              value={input}
